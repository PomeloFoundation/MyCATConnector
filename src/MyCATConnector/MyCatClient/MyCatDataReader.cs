...
namespace MyCat.Data.MyCatClient
{
	public sealed class MyCatDataReader : DbDataReader
	{
		...
		internal async Task<bool> NextResultAsync(IOBehavior ioBehavior, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (nextResult != null)
				{
					...
					// https://github.com/mysql-net/MyCatConnector/issues/135
		...
		internal MyCatCommand Command { get; private set; }
		internal MyCatConnection Connection => Command?.Connection;
		internal MyCatSession Session => Command?.Connection.Session;
