...
{
	public class MyCatTransaction : DbTransaction
	{
		...
		internal async Task CommitAsync(IOBehavior ioBehavior, CancellationToken cancellationToken)
		{
			...
			{
				using (var cmd = new MyCatCommand("commit", m_connection, this))
		...
		private void VerifyNotDisposed()
		{
			if (m_connection == null)
				throw new ObjectDisposedException(nameof(MyCatTransaction));
		}

		MyCatConnection m_connection;
		bool m_isFinished;
	}
}
