...
{
	internal sealed class MyCatStatementPreparer
	{
		...
		private sealed class ParameterSqlParser : MyCatParser
		{
			public ParameterSqlParser(MyCatStatementPreparer preparer, BinaryWriter writer)
			{
				m_preparer = preparer;
				m_writer = writer;
			}
			...
			protected override void OnNamedParameter(int index, int length)
			{
				...
				if (parameterIndex != -1)
					...
					throw new MyCatException("Parameter '{0}' must be defined.".FormatInvariant(parameterName));
			}
			...
			readonly MyCatStatementPreparer m_preparer;
		...
		readonly MyCatParameterCollection m_parameters;
