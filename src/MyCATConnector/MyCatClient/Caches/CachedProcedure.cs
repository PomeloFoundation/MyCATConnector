...
namespace MyCat.Data.MyCatClient.Caches
{
	internal class CachedProcedure
	{
		internal static async Task<CachedProcedure> FillAsync(IOBehavior ioBehavior, MyCatConnection connection, string schema, string component, CancellationToken cancellationToken)
		{
			var cmd = (MyCatCommand) connection.CreateCommand();
			...
			cmd.Parameters.Add(new MyCatParameter
			{
			...
			using (var reader = (MyCatDataReader) await cmd.ExecuteReaderAsync(CommandBehavior.Default, ioBehavior, cancellationToken).ConfigureAwait(false))
			{
