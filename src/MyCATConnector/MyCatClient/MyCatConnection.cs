...
{
	public sealed class MyCatConnection : DbConnection
	{
		...
		public new MyCatTransaction BeginTransaction() => (MyCatTransaction) base.BeginTransaction();

		public Task<MyCatTransaction> BeginTransactionAsync(CancellationToken cancellationToken = default(CancellationToken)) =>
			BeginDbTransactionAsync(IsolationLevel.Unspecified, AsyncIOBehavior, cancellationToken);

		public Task<MyCatTransaction> BeginTransactionAsync(IsolationLevel isolationLevel, CancellationToken cancellationToken = default(CancellationToken)) =>
		...
		private async Task OpenAsync(IOBehavior ioBehavior, CancellationToken cancellationToken)
		{
			...
			{
				SetState(ConnectionState.Closed);
				throw new MyCatException("Unable to connect to any of the specified MySQL hosts.", ex);
			}
		}
		...
		public override string ConnectionString
		{
			...
			{
				if (m_hasBeenOpened)
					throw new MyCatException("Cannot change connection string on a connection that has already been opened.");
				m_connectionStringBuilder = new MyCatConnectionStringBuilder(value);
		...
		public static Task ClearPoolAsync(MyCatConnection connection) => ClearPoolAsync(connection, connection.AsyncIOBehavior, CancellationToken.None);
		...
		protected override DbCommand CreateDbCommand() => new MyCatCommand(this, CurrentTransaction);

#if !NETSTANDARD1_6
		protected override DbProviderFactory DbProviderFactory => MyCatClientFactory.Instance;
#endif
		...
		internal MyCatTransaction CurrentTransaction { get; set; }
		...
		MyCatConnectionStringBuilder m_connectionStringBuilder;
		ConnectionSettings m_connectionSettings;
		MyCatSession m_session;
