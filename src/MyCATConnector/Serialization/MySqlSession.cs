...
{
	internal sealed class MyCatSession
	{
		public MyCatSession()
			: this(null, 0)
		{
		...
		public MyCatSession(ConnectionPool pool, int poolGeneration)
		{
			Pool = pool;
		...
		private void VerifyConnected()
		{
			if (m_state == State.Closed)
				throw new ObjectDisposedException(nameof(MyCatSession));
			if (m_state != State.Connected)
				throw new InvalidOperationException("MyCatSession is not connected.");
		}
